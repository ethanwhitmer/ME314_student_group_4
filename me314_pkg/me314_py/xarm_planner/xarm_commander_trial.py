...
class ME314_XArm_Queue_Commander(Node):
    def __init__(self):
        ...
        self.declare_parameter('ft_threshold', 30.0)
        ...
        self.cancel_command_sub = self.create_subscription(Bool, '/me314_xarm_cancel_command', self.cancel_command_callback, 10) # Added by Baxter
    ...
    ## Added By Baxter
    def cancel_command_callback(self,msg):
        if not self.is_executing or self.controller_goal_handle is None:
            return
        
        if msg.data:
            self.log_info("A node requested that the current command be canceled")
            ...
            with self.queue_lock:
                ...
                if self.command_queue:
                    ...
                    self.log_info("Command removed from queue")
